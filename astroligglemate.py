...
if st.button("(๑'ᵕ'๑)⸝*🔮 ดูดวงง!"):
    if not user_api_key:
        ...
        #สร้าง DataFrame
        category_order = ["การงาน", "การเงิน", "สุขภาพ", "ความรัก", "โชคลาภ", "Lucky Item ประจำสัปดาห์"]
        name_order = {friend["name"]: i for i, friend in enumerate(friends_data)}
        df["Name_Order"] = df["Name"].map(name_order)
        df["Category_Order"] = df["Category"].map(lambda x: category_order.index(x) if x in category_order else -1)
        df1 = df.sort_values(by=["Name_Order", "Category_Order"]).drop(columns=["Name_Order", "Category_Order"])
